...
const ShopContextProvider = (props) => {
  ...
  // Check if user is already logged in via cookie on app load
  ...
  const addToCart = async (itemId) => {
    ...
    if(token){
      try {
        const response = await axios.post(backend_url + "/api/cart/add", { itemId })
        if(response.data.success){
  ...
  const updateQuantity = async (itemId, qty) => {
    ...
    if(token){
      try {
        const response = await axios.post(backend_url + "/api/cart/update", { itemId, quantity: qty })
        if(response.data.success){
          toast.success(response.data.message)
        } else {
          toast.error(response.data.message)
  ...
  const contextValue = {
    books, currency, navigate, token, setToken,
    cartItems, setCartItems, addToCart, getCartCount,
    getCartAmount, updateQuantity, backend_url, userCart
  }
